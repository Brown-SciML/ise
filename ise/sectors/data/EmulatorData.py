...
class EmulatorData:
    ...
    def __init__(self, directory: str, spatial_grouping: str = "sectors"):
        ...
        ise.sectors.data.processors functions, converting IVAF to SLE, and saving initial condition data
        for later use.

        Args:
            directory (str): Directory containing processed files from ise.sectors.data.processors functions. Should contain master.csv.
    ...
    def create_boolean_indices(self, columns: str = "all"):
        ...
        if columns == "all":
            ...
            if not isinstance(columns, list):
                raise ValueError(
                    f"Columns argument must be of type: list, received {type(columns)}."
                )
    ...
    def drop_columns(self, columns: List[str]):
        ...
        if not isinstance(columns, list) and not isinstance(columns, str):
            raise ValueError(
                f"Columns argument must be of type: str|list, received {type(columns)}."
            )
    ...
    def scale(
        ...
        if "minmax" in scaler.lower():
            ...
            raise ValueError(
                f"scaler argument must be in ['MinMaxScaler', 'StandardScaler'], received {scaler}"
            )
        ...
        # TODO: Don't need this anymore with SLE as the prediction
            ...
            raise ValueError(
                f"values_type must be in ['inputs', 'outputs'], received {values_type}"
            )
