...
import pandas as pd
from ise.sectors.utils.utils import _structure_emulatordata_args
from ise.sectors.data import EmulatorData
...
def load_ml_data(data_directory: str, time_series: bool = True):
    ...
    functions in the ise.sectors.data.processing modules or process_data in the ise.sectors.pipelines.processing module.
...
def combine_testing_results(
    ...
    Args:
        ...
        bounds (dict | pd.DataFrame): Dictionary or pd.DataFrame of uncertainty bounds to be added to the dataframe, generally outputted from ise.sectors.models.testing.pretrained.test_pretrained_model. Defaults to None.
...
def create_distribution(year: int, dataset: np.ndarray):
    ...
    useful for calculating divergences such as ise.sectors.utils.data.kl_divergence and ise.sectors.utils.data.js_divergence.

    Args:
        year (int): Year to generate the distribution.
        dataset (np.ndarray): MX85 matrix of true values or predictions for the series, see ise.sectors.utils.data.group_by_run.
...
def calculate_distribution_metrics(
    ...
    """Wrapper for calculating distribution metrics from a dataset. Includes ise.sectors.utils.data.group_by_run to
    group the true values and predicted values into NXM matrices (with N=number of samples and
    M=85, or the number of years in the series). Then, it uses ise.sectors.utils.data.create_distribution to
