...
class Plotter:
    def __init__(self, results_dataset, column=None, condition=None, save_directory=None):
        ...
        self.trues, self.preds, self.scenarios = group_by_run(self.dataset, column=column, condition=condition)
    ...
    def plot_ensemble(self, uncertainty='quantiles', column=None, condition=None, save=None,):
        return ensemble.plot_ensemble(
            dataset=self.dataset, uncertainty=uncertainty, column=column, condition=condition, save=save, cache=self.cache,
        )
    ...
    def plot_test_series(self, model, data_directory, time_series=True, approx_dist=True, mc_iterations=100, confidence='95', draws='random', k=10, save_directory=None):
        if not isinstance(model, ise.models.timeseries.TimeSeriesEmulator):
            raise NotImplementedError('currently the only model compatible with this function is TimeSeriesEmulator.')
        self.model = model
        self.ml_directory = data_directory
        return testing.plot_test_series(
            model=model, data_directory=data_directory, time_series=time_series, approx_dist=approx_dist, mc_iterations=mc_iterations, 
    ...
    def plot_callibration(self, color_by=None, alpha=0.2, column=None, condition=None, save=None):
        return testing.plot_callibration(
            dataset=self.dataset, column=column, condition=condition, color_by=color_by, alpha=alpha, save=save
        )
