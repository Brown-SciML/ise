...
def test_forcing_data_exists():
    assert os.path.exists(
        FORCING_DIRECTORY
    ), "Forcing Directory doesn't exist. Download it from Globus Collection 'GHub-ISMIP6-Forcing'"
...
def test_forcing_correct_subfolders():
    assert all(
        [
            sub in os.listdir(FORCING_DIRECTORY)
            for sub in ["Ocean_Forcing", "Ice_Shelf_Fracture", "Atmosphere_Forcing"]
        ]
    ), "Forcing Directory does not contain the correct subdirectories."
...
def test_forcing_file_counts():
    ...
    assert (
        count == 3673
    ), "All files as found in Globus are not present. Functionality may be limited."
...
def test_grids_data_exists():
    assert os.path.exists(
        GRIDS_DIRECTORY
    ), "Grid Data Directory doesn't exist. Contact Helene Seroussi to get access."
...
def test_grids_correct_files():
    assert all(
        [
            sub in os.listdir(GRIDS_DIRECTORY)
            for sub in ["sectors_32km.nc", "sectors_16km.nc", "sectors_8km.nc", "sectors_4km.nc"]
        ]
    )
...
def test_output_data_exists():
    assert os.path.exists(
        ISMIP6_OUTPUT_DIRECTORY
    ), "ISMIP6 Output Directory doesn't exist. Download it from https://zenodo.org/record/3940766#.Y7yKwXZKhrp"
...
def test_output_correct_subfolders():
    assert all(
        [
            sub in os.listdir(ISMIP6_OUTPUT_DIRECTORY)
            for sub in [
    ...
    assert all(
        [
            sub in os.listdir(f"{ISMIP6_OUTPUT_DIRECTORY}/ComputedScalarsPaper/")
            for sub in [
                "JPL1",
                "VUB",
                "NCAR",
...
def test_output_file_counts():
    ...
    assert (
        count == 3335
    ), "All files as found in Zenodo are not present. Functionality may be limited."
