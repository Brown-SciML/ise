...
from ise.evaluation.metrics import mean_squared_error_sector, sum_by_sector
from ise.evaluation.plots import EvaluationPlotter
from ise.models.grid import DimensionProcessor, WeakPredictor
...
if not train and not os.path.exists(f"{dir_}/WeakPredictorModel_{experiment_description}.pth"):
    raise FileNotFoundError(
        f"{dir_}/WeakPredictorModel_{experiment_description}.pth does not exist. Please train the model first."
    )

grids_directory = f"/oscar/home/pvankatw/data/pvankatw/pvankatw-bfoxkemp/Grid_Files/"
grids_file = (
    f"{grids_directory}/AIS_sectors_8km.nc"
    if ice_sheet == "AIS"
    else f"{grids_directory}/GrIS_Basins_Rignot_sectors_5km.nc"
)
...
losses = dict(
    WeightedMSELoss=WeightedMSELoss(
        y.mean().mean(),
        y.values.flatten().std(),
    ),
    WeightedMSEPCALoss=WeightedMSEPCALoss(
        y.mean().mean(), y.values.flatten().std(), component_weights
    ),
    WeightedPCALoss=WeightedPCALoss(component_weights),
    HuberLoss=torch.nn.HuberLoss(),
    MSELoss=torch.nn.MSELoss(),
    WeightedMSELossWithSignPenalty=WeightedMSELossWithSignPenalty(
        y.mean().mean(), y.values.flatten().std(), weight_factor=1.0, sign_penalty_factor=0.01
    ),
)
...
if train:
    ...
    model.load_state_dict(
        torch.load(
            f"{dir_}/WeakPredictorModel_{experiment_description}.pth",
...
all_preds = (
    dim_processor.to_grid(model.predict(X)).reshape(-1, 761, 761)
    if ice_sheet == "AIS"
    else dim_processor.to_grid(model.predict(X)).reshape(-1, 337, 577)
)
all_trues = (
    dim_processor.to_grid(y).reshape(-1, 761, 761)
    if ice_sheet == "AIS"
    else dim_processor.to_grid(y).reshape(-1, 337, 577)
)
sector_mse = mean_squared_error_sector(
    sum_by_sector(all_trues, grids_file), sum_by_sector(all_preds, grids_file)
)
...
for i in [
    ...
    plotter = EvaluationPlotter(
        save_dir=f"/users/pvankatw/research/current/supplemental/more_training_experiments/{ice_sheet}/{dataset}/"
    )
    plotter.sector_side_by_side(
        sum_sectors_true,
        sum_sectors_pred,
        grids_file,
        outline_array_true=y_true,
        outline_array_pred=y_pred,
        timestep=86,
        save_path=f"{experiment_description}_mse{test_mse:0.2f}_sectors_{i}.png",
        cmap=plt.cm.viridis,
    )
    plotter.spatial_side_by_side(
        y_true.cpu(),
        y_pred.cpu(),
        timestep=86,
        save_path=f"{experiment_description}_mse{test_mse:0.2f}_{i}.png",
        cmap=plt.cm.viridis,
        video=False,
    )
