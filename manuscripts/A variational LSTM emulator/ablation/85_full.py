...
class YearlyModel(torch.nn.Module):
    ...
    def predict(self, x, mc_iterations=None,):
        ...
        if isinstance(x, np.ndarray):
            dataset = PyTorchDataset(torch.from_numpy(x).float(), None,)
        elif isinstance(x, torch.FloatTensor) or isinstance(x, torch.Tensor):
            dataset = PyTorchDataset(x.float(), None,)
...
print('Loading data...')
train_features, train_labels, test_features, test_labels, test_scenarios = load_ml_data(data_directory=r"/users/pvankatw/emulator/untracked_folder/ml_data",)
data_dict = {
...
for year in tqdm(train_features.year.unique()):
    ...
    year_df = pd.DataFrame(dict(preds=preds, std=sd))
