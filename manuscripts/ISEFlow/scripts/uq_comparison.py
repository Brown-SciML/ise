...
def get_dataset(ice_sheet="AIS"):
    ...
    all_data = all_data.drop_duplicates(
        subset=all_data.columns.difference(["set", "Scenario"])
    ).dropna()
    ...
    if ice_sheet == "AIS":
        for dataset in [train, val, test]:
            dataset["ice_shelf_collapse"] = (
                dataset["Ice shelf fracture_True"] == dataset["Ice shelf fracture_True"].max()
            )
    ...
    if ice_sheet == "AIS":
        cols = (
            ...
            if temp_only
            else ["smb_anomaly", spatial_unit, "ice_shelf_collapse", "year", "sle"]
            + ocean_sensitivity_columns
        )
    else:
        cols = (
            ["aST", "year", spatial_unit, "sle"] + ocean_sensitivity_columns
            if temp_only
            else ["aSMB", spatial_unit, "year", "sle"] + ocean_sensitivity_columns
        )
...
emulandice_temponly_ais = pd.read_csv(
    "/oscar/home/pvankatw/research/ise/manuscripts/ISEFlow/emulandice-ais-predictions.csv"
)
emulandice_temponly_gris = pd.read_csv(
    "/oscar/home/pvankatw/research/ise/manuscripts/ISEFlow/emulandice-gris-predictions.csv"
)

# Get datasets for AIS and GrIS
X_train_AIS, y_train_AIS, X_val_AIS, y_val_AIS, X_test_AIS, y_test_AIS = get_dataset(
    ice_sheet="AIS"
)
X_train_GrIS, y_train_GrIS, X_val_GrIS, y_val_GrIS, X_test_GrIS, y_test_GrIS = get_dataset(
    ice_sheet="GrIS"
)
...
smooth = False
iseflowAIS_temponly_preds, iseflowAIS_temponly_uq = iseflowAIS_temponly.predict(
    X_test_AIS, output_scaler=f"{AIS_data_dir}/scaler_y.pkl", smooth_projection=smooth
)
iseflowGrIS_temponly_preds, iseflowGrIS_temponly_uq = iseflowGrIS_temponly.predict(
    X_test_GrIS, output_scaler=f"{GrIS_data_dir}/scaler_y.pkl", smooth_projection=smooth
)
...
iseflowAIS_temponly["true"] = f.unscale_output(
    iseflowAIS_temponly["true"].values.reshape(-1, 1), f"{AIS_data_dir}/scaler_y.pkl"
)
iseflowGrIS_temponly["true"] = f.unscale_output(
    iseflowGrIS_temponly["true"].values.reshape(-1, 1), f"{GrIS_data_dir}/scaler_y.pkl"
)
...
def plot_projections(iseflow_data, emulandice_data, ice_sheet, save_dir, num_plots):
    ...
    i = 0
    while i <= num_plots:
        projection_starts = iseflow_data.loc[
            iseflow_data["year"] == iseflow_data.year.values[0]
        ].index.values
        ...
        ax1.plot(
            df_projection["year"], df_projection["true"], label="True", color="red", linewidth=2
        )
        ax1.plot(
            df_projection["year"],
            df_projection["pred"],
        ...
        ax1.fill_between(
            df_projection["year"],
            df_projection["pred"] - 1.96 * df_projection["uq_epistemic"],
            df_projection["pred"] + 1.96 * df_projection["uq_epistemic"],
            color="blue",
            alpha=0.5,
        )
        ax1.fill_between(
            df_projection["year"],
            df_projection["pred"] - 1.96 * df_projection["uq_total"],
            df_projection["pred"] + 1.96 * df_projection["uq_total"],
            color="blue",
            alpha=0.3,
        )
        ...
        ax2.plot(
            emulandice_projection["year"],
            emulandice_projection["pred"],
            label="Emulandice Predicted",
            color="green",
            linewidth=2,
        )
        ax2.plot(
        ...
        ax2.fill_between(
            emulandice_projection["year"],
            emulandice_projection["pred"]
            - 1.96 * np.abs(emulandice_projection.lower_bound - emulandice_projection.pred),
            emulandice_projection["pred"]
            + 1.96 * np.abs(emulandice_projection.lower_bound - emulandice_projection.pred),
            color="green",
            alpha=0.3,
...
plot_projections(
    iseflowAIS_temponly, emulandice_temponly_ais, "AIS", f"{out_dir}/AIS_region_temponly", 20
)
plot_projections(
    iseflowGrIS_temponly, emulandice_temponly_gris, "GrIS", f"{out_dir}/GrIS_region_temponly", 20
)

# CRPS calculation
i_ais_crps = np.median(
    crps(iseflowAIS_temponly["true"], iseflowAIS_temponly["pred"], iseflowAIS_temponly["uq_total"])
)
i_gris_crps = np.median(
    crps(
        iseflowGrIS_temponly["true"], iseflowGrIS_temponly["pred"], iseflowGrIS_temponly["uq_total"]
    )
)
e_ais_crps = np.median(
    crps(
        emulandice_temponly_ais["sle"],
        emulandice_temponly_ais["pred"],
        np.abs(emulandice_temponly_ais["lower_bound"] - emulandice_temponly_ais["pred"]),
    )
)
e_gris_crps = np.median(
    crps(
        emulandice_temponly_gris["sle"],
        emulandice_temponly_gris["pred"],
        np.abs(emulandice_temponly_gris["lower_bound"] - emulandice_temponly_gris["pred"]),
    )
)
