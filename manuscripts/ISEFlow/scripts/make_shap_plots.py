...
feature_names = [
    COLUMN_REMAPPING[x] if x in COLUMN_REMAPPING.keys() else x for x in shap_values.columns
]
...
feature_names = [
    COLUMN_REMAPPING[x] if x in COLUMN_REMAPPING.keys() else x for x in shap_agg.columns
]
...
if ICE_SHEET == "AIS":
    shap_values = pd.read_csv(
        r"/oscar/home/pvankatw/data/pvankatw/pvankatw-bfoxkemp/ISEFlow/explainability/SHAP/Scenario_target/AIS_shap_values_1724177548.840991.csv"
    )
else:
    shap_values = pd.read_csv(
        r"/oscar/home/pvankatw/data/pvankatw/pvankatw-bfoxkemp/ISEFlow/explainability/SHAP/Scenario_target/GrIS_shap_values_1724178272.421653.csv"
    )
...
print(
    f"Mean Temp SHAP Value ({ICE_SHEET}):",
    mean_shap_values["ts_anomaly" if ICE_SHEET == "AIS" else "aST"],
)
print(
    f"Sum of other SHAP Values ({ICE_SHEET}):",
    mean_shap_values.drop("ts_anomaly" if ICE_SHEET == "AIS" else "aST").sum(),
)
