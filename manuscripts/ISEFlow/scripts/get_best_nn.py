...
def get_best_nn(data_directory, export_directory, iterations=10, with_chars=True):
    X_train, y_train = f.get_X_y(
        pd.read_csv(f"{data_directory}/train.csv"),
        "sectors",
        return_format="numpy",
        with_chars=with_chars,
    )
    _, scenarios = f.get_X_y(
        pd.read_csv(f"{data_directory}/val.csv"),
        "scenario",
        return_format="pandas",
        with_chars=with_chars,
    )
    X_val_df, _ = f.get_X_y(
        pd.read_csv(f"{data_directory}/val.csv"),
        "sectors",
        return_format="pandas",
        with_chars=with_chars,
    ...
    for _ in range(iterations):
        for num_predictors in [10, 7, 5]:
            ...
            cur_time = time.time()
            de = DeepEnsemble(
                num_ensemble_members=num_predictors,
            ...
            emulator.fit(
                ...
                checkpoint_path=f"checkpoint_{ice_sheet}",
                early_stopping=True,
                patience=20,
                nf_epochs=nf_epochs,
                de_epochs=de_epochs,
            ...
            predictions, uncertainties = emulator.predict(
                X_val, output_scaler=f"{data_directory}/scaler_y.pkl"
            )
            y_val = f.unscale_output(y_val.reshape(-1, 1), f"{data_directory}/scaler_y.pkl")
            ...
            print(
                f"MAE: {mae}, MSE: {mse}, KLD: {kld}, JSD: {jsd}, KS_D: {ks_d}, KS_P: {ks_p}, Training Time: {total_train_time}"
            )
            metrics = dict(
...
if __name__ == "__main__":
    ...
    ITERATIONS = 1
    DATA_DIRECTORY = (
        f"/oscar/home/pvankatw/data/pvankatw/pvankatw-bfoxkemp/ISEFlow/data/ml/{ICE_SHEET}/"
    )
