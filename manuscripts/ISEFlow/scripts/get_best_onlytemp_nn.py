...
def get_optimal_temponly_model(ice_sheet, out_dir, iterations=10, with_chars=False):
    ...
    X_train, y_train = f.get_X_y(
        data, dataset_type="sectors", return_format="pandas", with_chars=with_chars
    )
    X_val, y_val = f.get_X_y(
        val_data, dataset_type="sectors", return_format="pandas", with_chars=with_chars
    )
    ...
    for _ in range(iterations):
        for num_predictors in [3, 5, 7, 10]:
            ...
            emulator.fit(
                ...
                checkpoint_path=f"{ice_sheet}_onlysmb_checkpoint.pt",
                early_stopping=True,
                patience=10,
                nf_epochs=nf_epochs,
                de_epochs=de_epochs,
            ...
            model_description = (
                f"npred{num_predictors}_nf{nf_epochs}_de{de_epochs}_withchars{with_chars}"
            )
            export_dir = f"{out_dir}/withchars{with_chars}/{model_description}_{cur_time}/"
            emulator.save(export_dir, input_features=cols)

            # Make predictions and save results
            predictions, uncertainties = emulator.predict(
                X_val, output_scaler=f"{data_dir}/scaler_y.pkl"
            )
            results = pd.DataFrame(
                dict(
                    year=years,
                    pred=predictions.squeeze(),
                    true=y_val_unscaled.squeeze(),
                    scenarios=scenarios.values.squeeze(),
                )
            )
            ...
            print(
                f"MAE: {mae}, MSE: {mse}, KS_D: {ks_d}, KS_P: {ks_p}, Training Time: {total_train_time}"
            )
...
if __name__ == "__main__":
    ...
    get_optimal_temponly_model(
        ice_sheet,
        "/users/pvankatw/research/ise/delete/",
        iterations=iterations,
        with_chars=with_chars,
    )
