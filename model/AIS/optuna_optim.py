...
def objective_factory(X_train, y_train, X_val, y_val):
    ...
    def objective(trial):
        ...
        os.makedirs(f"optuna_ckpts_z{z_dim}", exist_ok=True)
        ckpt_path = os.path.join(f"optuna_ckpts_z{z_dim}", f"trial_{trial.number}.pt")
...
def main(args):
    ...
    flow = NormalizingFlow.load(r"/users/pvankatw/research/ise/model/nf/nf.pt")
    flow.trained = True
