...
def randomize_parameters():
    params = {
        "batch_size": int(np.random.choice([128, 256, 512, 1024], p=[0.5/3, 0.5, 0.5/3, 0.5/3])),
        "dropout": np.random.choice([0.0, 0.1], p=[0.75, 0.25]),
        "layers": 1,
        "hidden_size": int(np.round(np.random.normal(loc=352, scale=128))),
        "sequence_length": np.random.choice([5, 10,], p=[0.75, 0.25]),
        "lr": sample_loguniform(5e-4, 5e-3),
        "wd": sample_loguniform(1e-5, 5e-5),
    }
    params['lr'] = params['lr'] * (params['batch_size'] / 256) ** 0.5 
    return params 
...
def main(args, params):
    ...
    save_dir = f"model/AIS/lstm/{model_id}"
...
if __name__ == "__main__":
    ...
    import json
    with open(r"/oscar/home/pvankatw/research/ise/model/AIS/ISEFlow_slc/deep_ensemble_metadata.json", 'r') as s:
    ...
    params = {
        "batch_size": 256,
        "dropout": 0,
        "layers": metadata["ensemble_members"][ensemble_member]['lstm_num_layers'],
        "hidden_size": metadata["ensemble_members"][ensemble_member]['lstm_num_hidden'],
        "sequence_length": metadata["ensemble_members"][ensemble_member]['sequence_length'],
        "lr": 5e-4,
        "wd": 1e-5,
        "epochs": metadata["ensemble_members"][ensemble_member]['epochs_trained'],
    }
    main(args, params)
