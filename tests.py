...
for lag in [None, 3, 5]:
    ...
    fe.split_data(
        ...
        val_size=0.15,
        test_size=0.15,
        output_directory=f"/oscar/scratch/pvankatw/pca/{ice_sheet}/",
    )

    data = fe.train

    X = data.drop(
        columns=[x for x in data.columns if "sle" in x] + ["cmip_model", "pathway", "exp", "id"]
    )
    ...
    for loss_name, loss in losses.items():
        ...
        for i in [10, 20, 28, 35, 40]:
            X = data.drop(
                columns=[x for x in data.columns if "sle" in x]
                + ["cmip_model", "pathway", "exp", "id"]
            )
            y = data[[x for x in data.columns if "sle" in x]]
            ...
            plotter = EvaluationPlotter(
                save_dir="/users/pvankatw/research/current/supplemental/loss_testing_loop/"
            )
            plotter.spatial_side_by_side(
                y_true.cpu(),
                y_pred.cpu(),
                timestep=86,
                save_path=f"{experiment_description}_{i}.png",
                cmap=plt.cm.viridis,
                video=False,
            )
