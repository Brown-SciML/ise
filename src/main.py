...
model = FC6_N256.FC6_N256(input_layer_size=X_train.shape[2])
...
for epoch in range(1, epochs+1):
    ...
    for i in range(X_test.shape[0]):
        ...
        X_test_batch = X_test[i, :, :]
        y_test_batch = y_test[i, :]
        
        test_pred = model(X_test_batch)
        loss = criterion(test_pred, y_test_batch.unsqueeze(1))
        test_total_loss += loss.item()
        
    test_loss = test_total_loss / X_test.shape[0]
...
model.eval()
X_test = torch.tensor(X_test, dtype=torch.float).reshape(-1, X_train.size()[2])
preds = model(X_test)
